...
2. Gather evidence: `sdlc-platform pr_get <id>` (title, `review_decision`, `merged_at`), `sdlc-platform pr_checks <id>` (`status`, check names), the newest `.sdlc/verify/*.md` and `*-security.md` (their Verdict and Blocking lines), `git log <previous tag>..HEAD --oneline` for the change list, `git describe --tags --abbrev=0` for the previous release. Version: `--version`, else ask.
...
6. Run the deploy command the project uses for production (the one matching `environments.prod.deployCommandPatterns`, typically `scripts/deploy.sh production <sha>` through the deploy workflow or pipeline). The `gate-production` hook now allows it for this commit until the marker expires. Report the outcome, update `RELEASE_AUTHORISED_BY` and `RELEASE_WATCH` in the release note, and tell the user to commit `.sdlc/releases/<version>.md` and tag the release. Done when the deploy command has run and the release note is complete.
