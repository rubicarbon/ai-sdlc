...
argument-hint: "[feature-dir] [--dry-run]"
...
Pass `--dry-run` through when the user asked for it and show the printed commands instead of publishing. Otherwise report the spec url and every ticket url from `publish-manifest.json`, then commit the feature directory with the message `Publish <slug> to <platform>`.
