...
| 8 | PR | verify PASS | `sdlc-platform pr_create <title> <body-file> <base> <head>`; a human code owner approves | the PR |
| 9 | Review on the PR | the PR | `review.runner` `ci` (default): the rendered review workflow comments; for the ship gate run `/ai-sdlc:sdlc-verify --security`. `local`: the `launch-local-review` hook has opened a separate session running `/ai-sdlc:sdlc-review --pr <id>` (its note names the launch); do not audit your own change here, wait for launch state `posted` (`bash "${CLAUDE_PLUGIN_ROOT}/scripts/review/status.sh" get --pr <id>`), address the findings, push, and the next review launches for the new head | the advisory comment; with `local` also `.sdlc/verify/<date>-<sha12>-pr<id>-security.md`, the only security evidence the ship gate accepts there |
...
Skip nothing between 5 and 10. Four hooks deny, one (`launch-local-review`, PostToolUse) only launches the local review. Hooks deny four things only: secret material, the paths in `guardrails.protectedPaths` plus the release-authorisation markers, file edits by the verifier and the auditor, and commands listed in `environments.prod.deployCommandPatterns` without a release authorisation. Everything else is process discipline: route through the stages instead of around them, because review and the ship gates read the artifacts each stage leaves behind.
