...
### `pr_get <id>`
```json
{"id":"77","title":"...","body":"...","state":"open","base":"main","head":"feature/x","base_sha":"<40 hex>","head_sha":"<40 hex>","head_repo":"owner/name","head_repo_url":"https://github.com/owner/name.git","is_fork":false,"url":"...","created_at":"...","merged_at":null,"closed_at":null,"additions":120,"deletions":8,"changed_files":4,"review_decision":"pending","author":"login","platform":"github"}
```
`state`: `open`, `merged`, `closed`. `review_decision`: `approved`, `changes_requested`, `pending`. Azure prints `null` for `additions`, `deletions`, `changed_files` (the CLI does not expose them; `metrics_export` computes them from git when possible). `body` is the PR description (`""` when empty). `head_sha` and `base_sha` are the commits the platform reports for the head and base branches at call time; `head_repo` (`owner/name` on GitHub, `project/repo` on Azure) and `head_repo_url` name the repository the head branch lives in, which is the fork on a cross-repository PR (`is_fork` true). A reviewer that must review exactly what the PR contains fetches `head_sha` from `head_repo_url` and checks the fetched commit against it (`scripts/review/snapshot.sh`).
...
- Azure: checks are the branch policy evaluations of the PR (`az repos pr policy list`: approver count, build, required reviewers, work-item linking, comment resolution); `required` is `[<required pipeline>]` where the required pipeline is `azure.pipelineName` when set (a custom build requirement, whatever the runner), else `sdlc-pr-review` when `review.runner` is `ci`, else nothing (`[]` with `review.runner: local` and no custom pipeline). Rule 3 still applies: the policy evaluations of the PR are the checks, so a PR with no evaluations at all fails closed.

Two settings, two rules, shared by init, the adapters and `pr_checks` (`review_runner` and `review_pipeline_name` in `_common.sh`): `review.runner` decides whether the review workflow files (`sdlc-pr-review.yml`, and `sdlc-cost-report.yml` on GitHub) are rendered, installed and registered at all; `azure.pipelineName` names a build requirement. `/ai-sdlc:sdlc-init` writes `pipelineName: "sdlc-pr-review"` for runner `ci` only and refuses runner `local` together with an explicit `pipelineName` of `sdlc-pr-review`.
...
```json
{"branch":"main","applied":["approver-count"],"updated":[],"unchanged":["comment-required"],"skipped":["build: pipeline 'sdlc-pr-review' is not registered yet (run ci_workflow_install first)"],"removed":[],"platform":"azure"}
```
`skipped` lists policies that could not be applied yet and why (GitHub always prints `[]`). `removed` lists obsolete policies the call deleted: with `review.runner: local` Azure deletes every build policy on the branch whose display name is `sdlc-pr-review` (the retired review pipeline), independent of a custom `azure.pipelineName`, and never any other build policy; GitHub always prints `[]` because its `PUT` replaces `required_status_checks.contexts` as a whole (a retired check shows up as an updated `contexts` field). A successful call (not `--dry-run`) marks `remote: "done"` in `<artifacts>/migrations.json` when a review-runner migration is pending there.
...
- With `review.runner: local` the review files are neither installed nor registered on either platform: `sdlc-pr-review.yml` (both) and `sdlc-cost-report.yml` (GitHub). This follows `review.runner` alone; a custom `azure.pipelineName` does not bring the review pipeline back.
...
```
Sources: GitHub uses a paginated GraphQL search over merged PRs with their reviews and commits, keeping the `metrics.maxPrs` most recent merges (default 200), the Deployments API (or workflow runs of `github.deployWorkflow` when it exists on the default branch), issues labelled per `metrics.incidentLabel`, and `git log --grep '^Revert'`. Azure uses completed PRs, PR threads for the first vote, runs of the configured deploy pipeline, a WIQL query for work items tagged with `metrics.incidentLabel`, and the same git log. Dates are ISO 8601 UTC.

`sources` says where each series came from so a reader can tell "zero" from "not measured": `prs` and `incidents` are always `configured`; `deployments` is `configured` on GitHub (the Deployments API always exists) and on Azure when `azure.deployPipelineId` is set or a pipeline named `azure.deployPipelineName` (default `sdlc-deploy`) exists, else `not-configured` with an empty series and a warning; `reverts` is `configured` when the local git history covers the period and `partial` when the clone is shallow (`git rev-parse --is-shallow-repository`), has no commit older than `<since>`, or `git log` fails, each with a warning. `warnings` is a list of strings; Azure adds `N of M pull requests have no local merge commits; size and first_commit_at are null for them` when the merge commits are not in the local clone, and GitHub adds one when `metrics.maxPrs` (or the search API's 1000-result ceiling) truncated the PR series, naming how many were merged and how many were kept, and one when `github.deployWorkflow` names a workflow that does not exist on the default branch (the Deployments API is used instead). The stdout summary repeats the same `warnings`.

Every CLI call must succeed and return JSON (see the exit-code table); the file is assembled in a temporary location and moved to `<out.json>` only at the end, so a failure leaves no file behind, and `--dry-run` writes no file at all. The single exception is a `github.deployWorkflow` that GitHub reports as absent from the default branch (`HTTP 404: workflow <name> not found on the default branch`): that is a configuration mistake with a correct fallback, so it becomes a warning and the Deployments API supplies the series. Any other `gh run list` failure — auth, rate limit, network — is still fatal. The Azure `az rest` calls carry the PAT in a header that never appears in a log line or error message.
