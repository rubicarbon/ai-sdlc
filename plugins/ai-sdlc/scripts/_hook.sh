...
# 3. Provides hook_deny, hook_rel, hook_cmd_paths, hook_list, hook_cmd_writes and hook_home.
...
# hook_list <jq path> [default...] : config array as lines. An absent key (or a non-array
# value) yields the defaults, an explicit [] yields nothing, a list yields its entries. One jq
# spawn; zero lines when there is nothing, so `mapfile -t` gives an empty array. An unreadable
...
# protected-paths guard combines it with hook_cmd_paths).
hook_cmd_writes() {
  local c="$1"
