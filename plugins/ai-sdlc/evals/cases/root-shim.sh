...
out=$(probe CLAUDE_PLUGIN_ROOT="$pn");           assert_eq "$pn|CLAUDE_PLUGIN_ROOT|0.1.2" "$out" "variable set to the real root is used"
out=$(probe -u CLAUDE_PLUGIN_ROOT);              assert_eq "$pn|script-path|0.1.2" "$out" "variable unset falls back to the script path"
out=$(probe CLAUDE_PLUGIN_ROOT=);                assert_eq "$pn|script-path|0.1.2" "$out" "variable empty falls back to the script path"
out=$(probe CLAUDE_PLUGIN_ROOT='${CLAUDE_PLUGIN_ROOT}'); assert_eq "$pn|script-path|0.1.2" "$out" "literal unexpanded placeholder falls back to the script path"
out=$(probe CLAUDE_PLUGIN_ROOT=/);               assert_eq "$pn|script-path|0.1.2" "$out" "variable '/' is rejected, never used"
out=$(probe CLAUDE_PLUGIN_ROOT="$EVAL_TMP");     assert_eq "$pn|script-path|0.1.2" "$out" "variable pointing at a non-plugin dir is rejected"
...
if [ -n "$win" ]; then
  out=$(probe CLAUDE_PLUGIN_ROOT="$win");        assert_eq "$pn|CLAUDE_PLUGIN_ROOT|0.1.2" "$out" "Windows path spelling normalises"
fi
