...
| Array | Source | Notes |
| --- | --- | --- |
| `prs` | `gh api graphql` over the search `repo:<repo> is:pr is:merged merged:<since>..<until>`, 100 per page to the end of the period (the search API stops at 1000), then sorted newest merge first and cut to `metrics.maxPrs` (default 200) | Asks for the PR scalars, `author.login`, `reviews(first: 100).submittedAt` and `commits(first: 100).commit.committedDate` only. `gh pr list --json author,reviews,commits` cannot be used: each commit carries an `authors` connection, which costs ~10,200 possible nodes per PR and GitHub rejects the query above ~49 of them. GraphQL search ignores `sort:` qualifiers, which is why the cap is applied locally rather than by asking for the first `maxPrs` results. `first_review_at` is the earliest `submittedAt` among the PR's review events; `first_commit_at` the earliest `committedDate` among its commits; `is_revert` is `title startswith "Revert"` |
| `deployments` | When `github.deployWorkflow` is set: `gh run list --workflow <name> --limit 200`, filtered to the period by `createdAt`; `status` is `success` for conclusion `success`, `pending` while the conclusion is null, `failure` otherwise. Otherwise: the Deployments API (`repos/<repo>/deployments?environment=<env>`, paginated), filtered by `created_at`, with the state of the most recent status per deployment (`success`; `pending`, `in_progress`, `queued` as pending; anything else as failure) | The rendered `sdlc-deploy.yml` writes a deployment record for every production run, so the API path works out of the box at tier 3. A `github.deployWorkflow` that does not exist on the default branch (nothing below tier 3 renders one) falls back to the Deployments API with a warning rather than failing the export; any other `gh run list` failure is still fatal |
...
- **Result limits.** GitHub exports the `metrics.maxPrs` most recently merged PRs of the period (default 200, drawn from at most the 1000 the search API will return), 200 workflow runs and 200 incident issues per export; Azure at most 500 completed PRs (then `metrics.maxPrs`, default 200), 200 pipeline runs. On GitHub a truncated PR series adds a `warnings` entry saying how many were merged and how many were kept; the other limits truncate silently.
