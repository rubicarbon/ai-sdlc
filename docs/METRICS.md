...
| Cost per merged PR | `total cost / merged PRs`, where total cost is summed from the `--cost-dir` files | The cost directory (see below) and at least one merged PR |

The report also prints `cost_total_usd` next to cost per merged PR.
...
The same eval checks that an export with empty arrays renders `n/a` everywhere instead of failing.
