...
The loop is grill, spec, tickets, build, review, verify, ship, learn. Each stage commits an artifact the next stage reads. Nothing is implemented without an accepted ticket, and nothing merges without a human at the gate; the first is a review rule, the second is branch protection. The tiers below add the artifacts and policies one layer at a time so the team can feel each one before the next arrives. Hooks stay the same at every tier and gate only two things: secret material, and production commands the project configured.

| Tier | Name | What `/ai-sdlc:sdlc-init` renders |
| --- | --- | --- |
| 0 | Foundation | A managed block in `CLAUDE.md`, an empty `CONTEXT.md` seed, `docs/agents/domain.md`, deny rules in `.claude/settings.json` (secrets, force push), marker lines in `.gitignore`, `sdlc.config.json`, `.sdlc/managed-files.json` |
...
| 2 | Review policy | No file or config change; the next-steps list tells you to run `sdlc-platform branch_protect_apply <default-branch>` |
...
Rendering is cumulative: tier 3 always includes tiers 0 to 2. The four guard hooks (`guard-secrets`, `guard-protected-paths`, `guard-verifier-readonly`, `gate-production`) and the `launch-local-review` PostToolUse hook (active only with `review.runner: local`, never denies) ship with the plugin and are the same at every tier; they exit silently in any repository without `sdlc.config.json`. Nothing gates editing source, tests, lockfiles, CI files or the config, installing packages, running scripts or deploying to staging.
...
| `/mattpocock-skills:implement` on a branch named for the ticket | you | Commits referencing the ticket id; `commands.format`, `commands.lint` and `commands.verify` run before the work is called done |
| `mattpocock-skills:code-review` | the agent, via the Skill tool | Review notes |
| `/ai-sdlc:sdlc-verify` | you | `.sdlc/verify/<date>-<sha>.md` with `**Verdict:**`, `**Commit:**` and `**Tree:**` lines, written by the `sdlc-verifier` subagent in a fresh context; it runs `commands.verify` directly on a clean checkout, or through `scripts/verify/run-isolated.sh` in a disposable worktree when the checkout is dirty (set `commands.verifySetup` when dependencies must be installed first) |
| `sdlc-platform pr_create <title> <body-file> <base> <head>` | the agent | The PR; a human approves it. With `review.runner: local` the `launch-local-review` hook now opens a separate session running `/ai-sdlc:sdlc-review --pr <id>`; the author waits for launch state `posted` |

Naming the ticket in the branch and the commits is a habit, not a hook: `REVIEW.md` closes a PR without a ticket id instead of reviewing it, and that is where the rule is held at every tier.
...
One thing changes: `next_steps` tells you to protect the default branch. No hook turns on and no config key changes.
...
### Two habits review holds

**The ticket.** Take the id from the ticket file's first line (`<!-- sdlc-publish: id=... -->`) or from the tracker, confirm with `sdlc-platform work_item_get <id>` that its blockers are `closed`, name it in the branch and every commit, and put it in the PR title or body. `REVIEW.md` says work without a ticket is closed, not reviewed. No marker file exists any more; nothing denies a source edit.

**The regression test.** Bug fixes follow `mattpocock-skills:diagnosing-bugs`: write the regression test, watch it fail, commit it red, then fix the production code without touching the test. If the test turns out to be wrong, change it in its own commit and say so in the PR, so the reviewer sees the test diff apart from the fix. Nothing denies a test edit; the reviewer checks that the test that went red is the one that went green.

**Protected paths, if you want them.** `guard-protected-paths` denies changes only to `.sdlc/release/**` and `.sdlc/UNLOCK_PROTECTED` by default. A project that wants its CI files, `CODEOWNERS`, lockfiles or `sdlc.config.json` hook-protected lists them in `guardrails.protectedPaths`; a human then unlocks a one-off change by creating `.sdlc/UNLOCK_PROTECTED` (which never unlocks the release markers) and deletes it afterwards.

What to measure at the end of week 4: how often a hook denied something, and whether each denial was the loop working or the configuration being wrong. Denials on a public certificate or an example file mean a fixture is named like a real secret: rename it, or set `guardrails.secretPaths` (and edit the deny list in `.claude/settings.json`, which the hook does not manage).
...
Before running it, set `cost.maxTurns`, `cost.maxBudgetUsd` and `cost.alertThresholdUsd` in `sdlc.config.json` (the example config uses 40 turns, 5 USD per run, 25 USD per week); they are substituted into the CI files. Tier 3 also needs a real `commands.verify`: it becomes the verification job that gates the branch, so `run.sh` exits 2 while the value is still the placeholder `detect.sh` proposes when a repository offers no candidate (that placeholder prints a reminder and exits 1, so a check built on it could never be meaningful). Pass `--verify "<cmd>"`. Tier 3 also needs the two deploy commands: `run.sh --tier 3 --deploy-staging "<cmd>" --deploy-production "<cmd>"` stores them as `commands.deployStaging` and `commands.deployProduction`, the deploy workflow runs exactly those commands with `SDLC_ENVIRONMENT` and `SDLC_SHA` exported, and the production command is added to `environments.prod.deployCommandPatterns` so the production gate covers it. Without them `run.sh` exits 2 with the flags to pass; `--no-deploy` sets up tier 3 without a deploy workflow. Then follow `next_steps`: commit the CI files, register them with `sdlc-platform ci_workflow_install`, and perform the human-only platform steps in `docs/PLATFORM-SETUP.md` (the `ANTHROPIC_API_KEY` secret or `sdlc-secrets` variable group when `review.runner` is `ci`, the `staging` and `production` environments with a human approval on `production`, build service permissions on Azure).

`--review-runner local` chooses the local review instead of the CI review: no review workflow and no API key; after `sdlc-platform pr_create` (or `gh pr create` / `az repos pr create`) the `launch-local-review` hook opens a terminal window running `/ai-sdlc:sdlc-review --pr <id>` in a fresh session with your own Claude login, which reviews the exact PR head in an isolated worktree, posts the report on the PR and publishes it as `.sdlc/verify/<date>-<sha12>-pr<id>-security.md`; a later push of an unreviewed commit launches the next review. `claude` must be on `PATH` in the terminal that runs Claude Code. On GitHub keep at least one CI workflow that reports a check on pull requests: `pr_checks` and the ship gate never pass with zero checks.

| File | GitHub | Azure | What it does |
| --- | --- | --- | --- |
| PR review (`review.runner: ci`) | `sdlc-pr-review.yml` | `sdlc-pr-review.yml` | Runs Claude Code on every PR with `--max-turns` and `--max-budget-usd` from the config, loads `ai-sdlc:sdlc-security-review`, posts an advisory comment ranked per `REVIEW.md` plus a spec-compliance section, records `sdlc-cost.json` as a build artifact, and fails the job when the run cost exceeds `cost.maxBudgetUsd`. Read-only tools only. The job also fails when the review did not complete: on Azure `scripts/ci/azure-review.sh` requires a successful `claude` result and a report with exactly one `Blocking: <n>` line, otherwise it posts a comment headed `ai-sdlc review FAILED` and exits 1; on GitHub a step fails the job when the action's result is missing, errored or hit a cap. A branch policy on the review pipeline therefore never turns green on a missing review. |
...
| Local review (`review.runner: local`) | none (hook) | none (hook) | Not a CI file: the `launch-local-review` hook opens the review session on the developer's machine; the review comment and the PR-bound report replace the workflow. Neither review workflow nor cost report is rendered or installed |
| Cost report (`review.runner: ci`) | `sdlc-cost-report.yml` | not rendered | Weekly: sums the last 7 days of `sdlc-cost-*` artifacts and opens or updates an issue labelled `sdlc-cost` when the total crosses `cost.alertThresholdUsd` |

The deploy gate on the agent's side is `gate-production`: any `Bash` or `PowerShell` command matching `environments.prod.deployCommandPatterns` is denied unless the release is authorised. The hook has no built-in patterns: `/ai-sdlc:sdlc-init` seeds `git push * <default branch>` and adds the configured production command, and you add `kubectl apply *`, `helm upgrade *`, `terraform apply *` or whatever else reaches production in your repository by hand. Projects initialised by an earlier plugin version keep the longer list that version seeded; `/ai-sdlc:sdlc-upgrade` never rewrites it. A matched command is denied unless `.sdlc/release/AUTHORIZED-<HEAD sha>` exists and is complete and valid: `authorised_by=`, `authorised_at=`, `expires=` and `commit=` each exactly once, a numeric expiry strictly in the future, and a full commit sha equal to `HEAD` and to the file name (`scripts/ship/_authz.sh`). That file is written by `scripts/ship/authorize.sh`, which refuses to run inside a Claude Code session and must be run by a human in their own terminal:
...
### Switching the review runner

`bash "${CLAUDE_PLUGIN_ROOT}/scripts/init/run.sh" --review-runner local` (or `ci`) on an initialised repository rewrites `review.runner`, `github.requiredChecks` and `azure.pipelineName` (a custom pipeline name is a build requirement and stays; the review pipeline name goes), then retires the files the new runner no longer renders: a review workflow that is byte-identical to what the plugin wrote is deleted (`retired`); an edited or never-recorded one is kept and reported `retire-pending` (`--check` stays red until you delete it or re-run with `--force`). The remote side does not follow by itself: `.sdlc/migrations.json` records the switch as `remote: pending` and `next_steps` repeat, on every run, `sdlc-platform branch_protect_apply <branch>` (GitHub replaces the required checks; Azure deletes the retired `sdlc-pr-review` build policy, reported under `removed`) and, on Azure, the deletion of the `sdlc-pr-review` pipeline definition, until `branch_protect_apply` has run and marked the migration `done`. `--dry-run` reports the retirements and writes nothing; `--only` limits the retirements but the config change always applies.

What to measure after tier 3: the CI review cost per PR (the step summary of every `sdlc-pr-review` run, and the weekly cost report; with `review.runner: local` there is no cost record, the review runs on your own login), how many review comments were acted on, and whether the evals workflow ever went red on drift. Drift means the plugin's templates moved; `run.sh --upgrade` applies the change, `--force` also overwrites user-edited files.
...
| Tier 2 | PRs closed for a missing ticket, test files changed in fix commits, revert rate | Both counts fall as habits form; reverts flat or down | The counts stay high after four weeks, or reviewers stop reading test diffs |
...
| Start building | `/mattpocock-skills:implement` on a branch named for the ticket; run `commands.format`, `commands.lint`, `commands.verify` before finishing |
| Test-first | `mattpocock-skills:tdd` (agent) |
| Bug fix | `mattpocock-skills:diagnosing-bugs` (agent); commit the red regression test first, fix without touching it |
...
| Review a PR locally (`review.runner: local`) | opened by the hook after `pr_create`, or by hand in a new terminal: `claude "/ai-sdlc:sdlc-review --pr <id>"`; state: `bash <plugin-root>/scripts/review/status.sh get --pr <id>` |
...
| `.gitignore` lines for `.sdlc/UNLOCK_PROTECTED`, `.sdlc/release/`, `.sdlc/tmp/` (older installs also have `.sdlc/ACTIVE_TICKET` and `.sdlc/FIX_MODE`, which nothing reads any more) | Delete the lines |
...
| Branch protection or branch policies, registered pipelines, GitHub environments, the `ANTHROPIC_API_KEY` secret (runner `ci`), the `sdlc-cost` label and issues | Platform settings; remove them in the platform UI or CLI |
| `.sdlc/tmp/review/` launch states, mappings and worktrees, `.sdlc/migrations.json` | Gitignored scratch of the local review runner (`scripts/review/status.sh sweep` bounds it) and the committed migration record; delete them |
