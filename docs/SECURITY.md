...
| `guard-verifier-readonly` hook | `hooks/guard-verifier-readonly.sh` | A compromised or over-eager verifier or security auditor cannot edit files, and its shell is an allowlist: read-only commands without redirection, four read-only `sdlc-platform` functions, `precondition.sh`, `validate-report.sh` and the isolation helper `scripts/verify/run-isolated.sh`. Scripts, interpreters, build tools, archives, downloads, git mutation and PowerShell writes are denied. The helper runs `commands.verify` in a disposable worktree and exits 4 when the main checkout's fingerprint changed, so an escape is detected and reported rather than assumed impossible |
| `guard-ticket-gate` and `guard-test-edits` on shell tools | `hooks/guard-ticket-gate.sh`, `hooks/guard-test-edits.sh` | The ticket gate and FIX_MODE cover Bash and PowerShell, not only the file tools: `sed -i`, `echo > file`, `tee`, `Set-Content`, `git add <source>` and `git checkout <path>` are checked against the same globs; scripts, interpreters, package managers, archives, downloads and dynamic targets (`$var`, globs, an unresolved `cd`) are denied because the guard cannot prove what they write |
| `gate-production` hook plus `authorize.sh` | `hooks/gate-production.sh`; `scripts/ship/authorize.sh` | Deploy-shaped commands (`environments.prod.deployCommandPatterns`, which always include the configured production command) run only while `.sdlc/release/AUTHORIZED-<HEAD sha>` is complete and valid: `authorised_by=`, `authorised_at=`, `expires=` and `commit=` each exactly once, a numeric expiry strictly in the future, a full sha equal to HEAD and to the file name (`scripts/ship/_authz.sh`, shared with `preflight.sh`). The marker is written by a human running `authorize.sh` at their own terminal: the script exits 2 when `CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT` or `CLAUDE_PROJECT_DIR` is set, and the directory is a protected path |
...
| Release evidence validation | `scripts/loop/_reports.sh`, used by `precondition.sh ship`, `preflight.sh` and `validate-report.sh` | A verification report counts only when it is a normal report (never `*-security.md`), non-empty, with exactly one `Verdict: PASS` and one `Commit:` equal to HEAD; a security report counts only with exactly one parseable `Blocking: <n>` line (and a matching `Commit:` when present). Empty, duplicated, malformed or stale evidence is a red gate, never zero findings |
...
- Paths constructed at runtime inside a Bash command (a variable expanded by the shell, `base64 -d | sh`, `eval` of a computed string, a path built by a script the command runs). `guard-secrets` and `guard-protected-paths` inspect the command text as written; they cannot evaluate it. The ticket gate, the test guard and the verifier guard take the opposite stance for the same reason: a target they cannot resolve, or a command whose writes they cannot see, is denied.
- The verifier isolation helper is directory-level isolation with after-the-fact proof. A verification command that writes into the main checkout by absolute path is detected by the fingerprint comparison (exit 4) and reported; it is not prevented, and changes to gitignored files are outside the fingerprint.
