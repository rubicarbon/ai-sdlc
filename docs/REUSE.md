...
| Build | `implement` | U | Builds from spec or tickets, drives `/tdd` at pre-agreed seams, closes with `/code-review`, commits to the current branch | Guardrails: nothing prevents editing tests to pass, touching protected files, reading secrets, or deploying | `guard-test-edits.sh`, `guard-protected-paths.sh`, `guard-secrets.sh`, `gate-production.sh`; `sdlc-loop` requires accepted tickets before this stage |
| Test | `tdd` | M | Red → green loop, seams agreed up front, anti-patterns | Deterministic enforcement | `guard-test-edits.sh` is the backstop under `FIX_MODE` |
| Test | `diagnosing-bugs` | M | Gated loop: build a red feedback loop, reproduce, minimise, hypothesise, instrument, fix, regression test, clean up | Preventing the fix from editing the regression test | `sdlc-loop` FIX_MODE protocol: after the regression test is red and committed, `touch .sdlc/FIX_MODE`; the hook denies test edits until it is removed |
...
| Support | `wizard` | M | Generates a bash wizard for steps only a human can perform (URLs, secret capture, `.env` and `gh secret` writes) | Azure DevOps variable groups / service connections | `sdlc-init` calls it to generate `scripts/sdlc-setup-wizard.sh`; the generated stages use `az pipelines variable-group` where the platform is Azure |
...
- `skills/misc/setup-pre-commit` (not shipped) installs husky + lint-staged. `post-edit-verify.sh` runs the configured formatter/linter at edit time instead and does not touch the repo's commit hooks.
